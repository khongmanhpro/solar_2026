...
export interface AdminLeadListItem {
  ...
  calculation: {
    ...
    electricalPhase: AdminLeadDetail["calculation"]["electricalPhase"];
    province: string;
  ...
  return (
    <section className="admin-panel scroll-mt-6 overflow-hidden" id="lead-detail">
      ...
      <div className="grid gap-px bg-[var(--line)] xl:grid-cols-3">
        ...
        <div className="bg-[var(--paper)] p-5 sm:p-7"><p className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--brand)]">Đầu vào calculation</p><dl className="mt-5 space-y-4 text-sm"><DetailRow label="Loại điện" value={ELECTRICITY_TYPE_LABELS[detail.calculation.electricityType]} /><DetailRow label="Pha điện" value={detail.calculation.electricalPhase === "single-phase" ? "Điện 1 pha" : detail.calculation.electricalPhase === "three-phase" ? "Điện 3 pha" : "Chưa thu thập"} /><DetailRow label="Tiền điện" value={formatVnd(detail.calculation.monthlyBill)} /><DetailRow label="Khu vực" value={detail.calculation.province} /><DetailRow label="Dùng ban ngày" value={DAYTIME_USAGE_LABELS[detail.calculation.daytimeUsageLevel]} /><DetailRow label="Diện tích mái" value={detail.calculation.roofAreaM2 === null ? "Chưa biết — cần khảo sát" : `${detail.calculation.roofAreaM2} m²`} /><DetailRow label="Điện dự phòng" value={detail.calculation.backupRequired ? "Có" : "Không"} /></dl></div>
